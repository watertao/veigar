...
[NOTE]
====
手动添加的 Mapper 接口必须要添加 `@Mapper` 和 `@Component` 注解，且 `@Component` 注解的名称设定为 Mapper 接口的全限定名：
...
@Component("com.mycompany.myapp.mapper.MyCustomMapper")
@Mapper
public interface MyCustomMapper {
  ...
}
----
这么做的目的主要是为了避免不同 package 下相同类名的 Mapper 接口产生冲突。
